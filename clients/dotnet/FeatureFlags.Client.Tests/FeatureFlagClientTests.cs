...
public class FeatureFlagClientTests
{
    ...
    private FeatureFlagClient CreateSut()
    {
    ...
    public async Task RefreshAsync_ShouldReportAFailureRatherThanSwallowIt()
    {
        ...
        await Assert.ThrowsAsync<FeatureFlagsException>(() => CreateSut().RefreshAsync(Cancellation));
    }
    ...
    public async Task RefreshAsync_WithARejectedKey_ShouldSaySo()
    {
        ...
        var exception = await Assert.ThrowsAsync<FeatureFlagsException>(
            () => CreateSut().RefreshAsync(Cancellation));
