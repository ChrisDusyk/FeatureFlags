using FeatureFlags.Domain.Flags;

namespace FeatureFlags.Server.Features.Flags.UpdateFlag;

// Slice-qualified rather than plain FlagStateResponse: AddOpenApi()'s default schema-ID
// generation keys on the bare type name, and this shape is declared once per slice (see
// CreateFlagResponse.cs, GetFlagResponse.cs) — an unqualified name here would collide with
// theirs and silently collapse to whichever one the generator happened to see first.
public sealed record UpdateFlagStateResponse(string Environment, bool IsEnabled, DateTimeOffset UpdatedAt);

public sealed record UpdateFlagResponse(
    Guid Id,
    string Key,
    string Name,
    string Description,
    DateTimeOffset CreatedAt,
    DateTimeOffset UpdatedAt,
    IReadOnlyList<UpdateFlagStateResponse> States)
{
    public static UpdateFlagResponse From(FeatureFlag flag) => new(
        flag.Id,
        flag.Key.Value,
        flag.Name,
        flag.Description,
        flag.CreatedAt,
        flag.UpdatedAt,
        [.. flag.States.Select(state =>
            new UpdateFlagStateResponse(state.Environment.Value, state.IsEnabled, state.UpdatedAt))]);
}
