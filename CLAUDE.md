...
- **Two kinds of credential, one header, and the policies are what keep them apart.** A user's JWT and an `SdkKey` both arrive as `Authorization: Bearer`; a policy scheme (`AuthSchemes.Any`) forwards on the `ffs_` prefix, which is a total test because a JWT cannot start with it. **Every policy names its scheme** — `RequireAuthenticatedUser()` is satisfied by any authenticated principal, so without `AddAuthenticationSchemes` an SDK key would pass `SignedIn` and be handed the whole console API. Do not remove that pinning, and give a new policy a scheme when you add one. `AuthPoliciesTests` is the guard.
